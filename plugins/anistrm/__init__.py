...
class ANiStrm(_PluginBase):
    ...
    plugin_version = "2.3"
    # 插件作者
    plugin_author = "kangbeicai"
    # 作者主页
    author_url = "https://github.com/kangbeicai"
    # 插件配置项ID前缀
    ...
    def get_current_season_list(self) -> List:
        url = f'https://aniopen.kangbeicai.top/{self.__get_ani_season()}/'
    ...
    def get_latest_list(self) -> List:
        ...
        for item in items:
            ...
            rss_info['link'] = link.replace("resources.ani.rip", "aniopen.kangbeicai.top")
            ret_array.append(rss_info)
        return ret_array

    def __touch_strm_file(self, file_name, file_url: str = None) -> bool:
        if not file_url:
            src_url = f'https://aniopen.kangbeicai.top/{self._date}/{file_name}?d=true'
        else:
            src_url = file_url
